...
+ to switch between access levels we use *CONTROL* register
...
this is an ARM register which stands for *Interrupt Program Status Register*. If this register's first 9 bits are zero, this means the program is in *Thread Mode*. Otherwise in *Handler Mode*
