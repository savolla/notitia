...
typedef struct A {
  ...
  #+end_src
the valid version of the code abovo is:
  #+begin_src c
typedef struct A {
    int x;
}
  #+end_src
**** struct sizes doesn't make sense
...
1. add the *url* of the site that your want to scrape.
    #+begin_src py
url = "https://www.newegg.com/global/tr-en/p/pl?d=graphics+card"
    #+end_src
2. get the page from the internet. this might take a while depending on your internet connection and size of the page
   #+begin_src py
uClient = uReq(url)
   #+end_src
...
page_soup = soup(page_html, "html.parser")
   #+end_src
***** ---------
now we can get every html element value like this;
#+begin_src py
print( page_soup.h1 )
#+end_src
...
#+begin_quote
this is not secure! your password and username will be stores inside *.git* directory in *planetext*. enyone on your PC will have access to them
#+end_quote
...
after this operation, git won't ask about username and password for this repository. this is not a global modification btw. you can do it for your repositories one by one
*** stow
...
:END:
Here I made some album recommendations for you. This is what i listen to when i really want to get into the mood. i made my best by separating genres into two categories. this list will grow up. I'll trow goodies here when i think it's worthy
** music for normies
this is *normal music for normal people*. (see what I did?)
...
** danger zone
=WARNING= this list is not for pregrant ladies or people with hearth conditions. I'll list the following content from most "normie" to *deepest depths of hell*
...
+ [[https://www.youtube.com/watch?v=4z9X0htC3mg][Modern Day Babylon - Travelers]]
  my first djent album. this is the first metal album without vocals
...
+ [[https://www.youtube.com/watch?v=TTs9AUo5R3U][Dissonance In Design - Sentient ]]
  technical death albums suppose to contain some imposible riffs right? but this album breaks this rule. and it sounds so good!
...
+ [[https://www.youtube.com/watch?v=dWxd05N8zXc][Carach Angren - Death Came Through a Phantom Ship]]
...
+ [[https://www.youtube.com/watch?v=szk5vGqPn2U][The LONGING - Original Soundtrack]]
  depressed and sad melodies..
+ [[https://www.youtube.com/watch?v=67BcCqnn8-s][Thy Light - Suici.De.pression]]
  enabling *depression mode* of yours. skip to *7:20*
...
+ [[https://cryptopsyofficial.bandcamp.com/album/none-so-vile][Cryptopsy - None So Vile]]
  old and good Cryptopsy.
+ [[https://www.youtube.com/watch?v=xGGK93eqAMo][Abominable Putridity - The Anomalies Of Artificial Origin]]
  Russians really know how to do it
+ [[https://severetorture.bandcamp.com/][Severe Torture - Slaughtered]]
  brutality takes another form
+ [[https://www.youtube.com/watch?v=dheNB2erMZI][Horrific - Your Worst Nightmare]]
  very underrated album.I'm not gonna call this album a metal album. this is like introduction to metal music. songs in this album have 80's rock flavor. Guitars are not in tune and songs are played very sloppy. contains funny elements.. great album
* Template
** facts
** concept / term
** tutorial / howto
