...
<link href="./style.css" rel="stylesheet"/>
   #+END_SRC
 + write internal css?
   put css code into <style> elements which is inside <head> element
   #+BEGIN_SRC html
<head>
  <style>
    h3 {
   ...
   #+END_SRC
 + how comments look in css?
   #+BEGIN_SRC css
/* this is a comment */
   #+END_SRC
 + change h1,h2 and h3 at the same *selector*?
   #+BEGIN_SRC css
h1,h2,h3 {
    color : olive;
}
...
.class0 {
   ...
   #+END_SRC
 + make *links* without _underline_ ?
   #+BEGIN_SRC css
a {
    text-decoration : none;
}
   #+END_SRC
 + cancel all the *default margins*?
   #+BEGIN_SRC css
   *{
       margin : 0;
   }
   #+END_SRC
 + *change font* of an element?
   #+BEGIN_SRC css
.element {
...
.element {
   ...
   #+END_SRC
 + reach a *nested element* inside a class?
   use *>* syntax
   #+BEGIN_SRC html
<div class="cls0">
  <ul>
...
cls0 > ul > li {
   ...
   #+END_SRC
 + increase space between *letters* in a paragraph?
   #+BEGIN_SRC css
.element {
    letter-spacing: 5px;
}
...
.element {
   ...
   #+END_SRC
 + capitalize an entire *paragraph* ?
   #+BEGIN_SRC css
.element {
    text-transform: capitalize;
}
   #+END_SRC
 + add *hover* action to an element?
    #+BEGIN_SRC css
    .element:hover {
    /* code */
    }
...
.element {
   ...
   #+END_SRC
 + add *border width* of an element?
   #+BEGIN_SRC css
.element {
    border-width: 10px;
}
...
ul > li {
   ...
   #+END_SRC
 + display text in the center of the element?
...
.element {
   ...
   #+END_SRC
 + use *image* as a background instead of straight color?
   #+BEGIN_SRC css
.element {
    background: url("/path/to/image.extension");
}
   #+END_SRC
 + make an image non-repeatable?
   #+BEGIN_SRC css
.element {
    background: url("/path/to/image.extension");
   ...
   #+END_SRC
 + *spread* the image into one area (div) ?
   #+BEGIN_SRC css
.element {
    background: url("/path/to/image.extension");
...
.element {
   ...
   #+END_SRC
 + adjust the position by x and y values of a background image?
...
.element {
   ...
   #+END_SRC
 + add an image with protected *aspect ratio*?
   #+BEGIN_SRC css
.element {
...
.banner {
   ...
   #+END_SRC
 + make a gradient element? use *grey* and *white*
   #+BEGIN_SRC css
.element {
    background-attachment: linear-gradient(grey, white);
}
   #+END_SRC
 + give *angle* to the gradient?
   #+BEGIN_SRC css
.element {
    background: linear-gradient(45deg, white, black);
}
   #+END_SRC
 + make a nice *transition effect* ? when user scrolls down the transparent
